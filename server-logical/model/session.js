...
function session_status(id) {
    ...
    if (historysession[id]) return HISTORY;
    return BAD;
}

function try_to_get_session(id) {
    if (activesessions.data[id]) return activesessions.data[id];
    if (historysession[id]) return historysession[id];
}

function __active_session_down(i) {
    //letting user know
    var session_obj = activesessions.data[i]
    if (activesessions.data[i] && !historysession[i]) {
        historysession[i] = JSON.parse(JSON.stringify(activesessions.data[i]));
...
function session_sync(id, session_obj) {
    ...
    if (state == ACTIVE) {
        if (JSON.stringify(activesessions.data[id][i]) != JSON.stringify(session_obj[i])) {
...
machines.events.on('states', (changes) => {
    ...
    console.warn("Heavy Op: Looping through devices during all updates!!")
...
var SESSION_REQ_EXPIRE = 1000;

function request_new_session(uid, machine_id, cost) {
    //valid?
    var user = users.db[uid];
